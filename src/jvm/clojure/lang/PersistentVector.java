...
public class PersistentVector extends APersistentVector
  ...
  public static PersistentVector create(ISeq items) {
    ...
    int i = 0;
    for (; items != null && i < 32; items = items.next()) arr[i++] = items.first();
    ...
    if (items != null) { // >32, construct with array directly
      ...
      for (; items != null; items = items.next()) ret = ret.conj(items.first());
      return ret.persistent();
  ...
  public static PersistentVector create(List list) {
    ...
    TransientVector ret = EMPTY.asTransient();
    for (int i = 0; i < size; i++) ret = ret.conj(list.get(i));
    return ret.persistent();
  }

  public static PersistentVector create(Iterable items) {
    // optimize common case
    if (items instanceof ArrayList) return create((ArrayList) items);

    Iterator iter = items.iterator();
    TransientVector ret = EMPTY.asTransient();
    while (iter.hasNext()) ret = ret.conj(iter.next());
    return ret.persistent();
  }

  public static PersistentVector create(Object... items) {
    TransientVector ret = EMPTY.asTransient();
    for (Object item : items) ret = ret.conj(item);
    return ret.persistent();
  }
  ...
  public static final class ChunkedSeq extends ASeq implements IChunkedSeq, Counted {
    ...
    public ISeq chunkedNext() {
      if (i + node.length < vec.cnt) return new ChunkedSeq(vec, i + node.length, 0);
      return null;
    }
    ...
    public ISeq next() {
      if (offset + 1 < node.length) return new ChunkedSeq(vec, node, i, offset + 1);
      return chunkedNext();
    }
  ...
  static final class TransientVector extends AFn
    ...
    public final IMapEntry entryAt(Object key) {
      ...
      if (v != NOT_FOUND) return MapEntry.create(key, v);
      return null;
    }
