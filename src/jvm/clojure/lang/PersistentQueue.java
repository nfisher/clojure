...
/**
 * conses onto rear, peeks/pops from front See Okasaki's Batched Queues This differs in that it uses
 * a PersistentVector as the rear, which is in-order, so no reversing or suspensions required for
 * persistent use
 */
    ...
    return new Iterator() {
      ...
      public Object next() {
        if (fseq != null) {
          ...
          fseq = fseq.next();
          return ret;
        } else if (riter != null && riter.hasNext()) return riter.next();
