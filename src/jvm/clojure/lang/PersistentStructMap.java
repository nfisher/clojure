...
public class PersistentStructMap extends APersistentMap implements IObj {
    ...
    if (e != null) {
      ...
      return new AFn() {
        public Object invoke(Object arg1) {
          ...
          if (m.def != def) throw Util.runtimeException("Accessor/struct mismatch");
          return m.vals[i];
        }
      };
    }
  ...
  /**
   * Returns a new instance of PersistentStructMap using the given parameters. This function is used
   * instead of the PersistentStructMap constructor by all methods that return a new
   * PersistentStructMap. This is done so as to allow subclasses to return instances of their class
   * from all PersistentStructMap methods.
   */
  protected PersistentStructMap makeNew(
  ...
  public IMapEntry entryAt(Object key) {
    Map.Entry e = def.keyslots.entryAt(key);
    if (e != null) {
      return MapEntry.create(e.getKey(), vals[(Integer) e.getValue()]);
    }
    ...
    return new Iterator() {
      ...
      public Object next() {
        if (ks != null) {
          ...
          ks = ks.next();
          return entryAt(key);
        } else if (extIter != null && extIter.hasNext()) return extIter.next();
  ...
  static class Seq extends ASeq {
    ...
    public ISeq next() {
      if (i + 1 < vals.length) return new Seq(_meta, keys.next(), vals, i + 1, ext);
      return ext.seq();
    }
  }
}
