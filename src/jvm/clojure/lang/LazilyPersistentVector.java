...
public class LazilyPersistentVector {
  ...
  static int fcount(Object c) {
    if (c instanceof Counted) return ((Counted) c).count();
  ...
  public static IPersistentVector create(Object obj) {
    ...
    //   else
    if (obj instanceof IReduceInit) return PersistentVector.create((IReduceInit) obj);
    else if (obj instanceof ISeq) return PersistentVector.create(RT.seq(obj));
    else if (obj instanceof Iterable) return PersistentVector.create((Iterable) obj);
    else return createOwning(RT.toArray(obj));
  }
}
