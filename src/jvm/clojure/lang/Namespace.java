...
public class Namespace extends AReference implements Serializable {
  static final ConcurrentHashMap<Symbol, Namespace> namespaces =
      new ConcurrentHashMap<Symbol, Namespace>();
  public final Symbol name;
  final transient AtomicReference<IPersistentMap> mappings = new AtomicReference<IPersistentMap>();
  final transient AtomicReference<IPersistentMap> aliases = new AtomicReference<IPersistentMap>();
  ...
  public Var intern(Symbol sym) {
    ...
    while (!mappings.compareAndSet(map, map.assoc(sym, v))) map = getMappings();
  ...
  Object reference(Symbol sym, Object val) {
    ...
    while (!mappings.compareAndSet(map, map.assoc(sym, val))) map = getMappings();
  ...
  public void addAlias(Symbol alias, Namespace ns) {
    if (alias == null || ns == null) throw new NullPointerException("Expecting Symbol + Namespace");
    IPersistentMap map = getAliases();
