 ...
 *
 * <p>Copies array on every change, so only appropriate for _very_small_ maps
 *
 * <p>null keys and values are ok, but you won't be able to distinguish a null value via valAt - use
 * contains/entryAt
 */
public class PersistentArrayMap extends APersistentMap
  ...
  public static PersistentArrayMap createAsIfByAssoc(Object[] init) {
    if ((init.length & 1) == 1)
      throw new IllegalArgumentException(
          String.format("No value supplied for key: %s", init[init.length - 1]));
  ...
  public IMapEntry entryAt(Object key) {
    int i = indexOf(key);
    if (i >= 0) return MapEntry.create(array[i], array[i + 1]);
    return null;
  }
  ...
  public IPersistentMap assocEx(Object key, Object val) {
    ...
    {
      if (array.length >= HASHTABLE_THRESHOLD) return createHT(array).assocEx(key, val);
  ...
  public IPersistentMap assoc(Object key, Object val) {
    ...
    {
      if (array.length >= HASHTABLE_THRESHOLD) return createHT(array).assoc(key, val);
      newArray = new Object[array.length + 2];
  ...
  static final class TransientArrayMap extends ATransientMap {
    ...
    void ensureEditable() {
      if (owner == null) throw new IllegalAccessError("Transient used after persistent! call");
    }
  }
}
