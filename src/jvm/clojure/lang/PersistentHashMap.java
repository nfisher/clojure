...
public class PersistentHashMap extends APersistentMap
  ...
  public PersistentHashMap(
      IPersistentMap meta, int count, INode root, boolean hasNull, Object nullValue) {
    this._meta = meta;
    this.count = count;
    this.root = root;
    this.hasNull = hasNull;
  ...
  public static PersistentHashMap createWithCheck(Object... init) {
    ...
    for (int i = 0; i < init.length; i += 2) {
      ret = ret.assoc(init[i], init[i + 1]);
      if (ret.count() != i / 2 + 1) throw new IllegalArgumentException("Duplicate key: " + init[i]);
    }
    return (PersistentHashMap) ret.persistent();
  }
  ...
  private static INode createNode(
      int shift, Object key1, Object val1, int key2hash, Object key2, Object val2) {
  ...
  public IMapEntry entryAt(Object key) {
    if (key == null) return hasNull ? MapEntry.create(null, nullValue) : null;
  ...
  public IPersistentMap assoc(Object key, Object val) {
    ...
    Box addedLeaf = new Box(null);
    INode newroot =
        (root == null ? BitmapIndexedNode.EMPTY : root).assoc(0, hash(key), key, val, addedLeaf);
    if (newroot == root) return this;
    return new PersistentHashMap(
        meta(), addedLeaf.val == null ? count : count + 1, newroot, hasNull, nullValue);
  }
  ...
  */
  ...
  interface INode extends Serializable {
    ...
    INode assoc(
        AtomicReference<Thread> edit, int shift, int hash, Object key, Object val, Box addedLeaf);
  ...
  static final class TransientHashMap extends ATransientMap {
    ...
    TransientHashMap(
        AtomicReference<Thread> edit, INode root, int count, boolean hasNull, Object nullValue) {
      this.edit = edit;
      this.root = root;
      this.count = count;
      this.hasNull = hasNull;
    ...
    ITransientMap doAssoc(Object key, Object val) {
      ...
      leafFlag.val = null;
      INode n =
          (root == null ? BitmapIndexedNode.EMPTY : root)
    ...
    void ensureEditable() {
      if (edit.get() == null) throw new IllegalAccessError("Transient used after persistent! call");
    }
  }

  static final class ArrayNode implements INode {
    final INode[] array;
    final AtomicReference<Thread> edit;
    int count;
    ...
    public static Object foldTasks(
        List<Callable> tasks,
        final IFn combinef,
        final IFn fjtask,
        final IFn fjfork,
      ...
      return combinef.invoke(
          foldTasks(t1, combinef, fjtask, fjfork, fjjoin), fjjoin.invoke(forked));
    }
    ...
    public INode assoc(int shift, int hash, Object key, Object val, Box addedLeaf) {
      ...
      if (node == null)
        return new ArrayNode(
            ...
            count + 1,
            cloneAndSet(
                array, idx, BitmapIndexedNode.EMPTY.assoc(shift + 5, hash, key, val, addedLeaf)));
    ...
    public INode without(
        AtomicReference<Thread> edit, int shift, int hash, Object key, Box removedLeaf) {
      int idx = mask(hash, shift);
      INode node = array[idx];
      if (node == null) return this;
    ...
    static class Iter implements Iterator {
      ...
      public boolean hasNext() {
        while (true) {
          if (nestedIter != null)
            if (nestedIter.hasNext()) return true;
            else nestedIter = null;

          if (i < array.length) {
            INode node = array[i++];
            if (node != null) nestedIter = node.iterator(f);
          } else return false;
        }
      }

      public Object next() {
        if (hasNext()) return nestedIter.next();
  ...
  static final class BitmapIndexedNode implements INode {
          ...
          for (int i = 0; i < 32; i++)
            if (((bitmap >>> i) & 1) != 0) {
              if (array[j] == null) nodes[i] = (INode) array[j + 1];
                ...
                nodes[i] =
                    EMPTY.assoc(shift + 5, hash(array[j]), array[j], array[j + 1], addedLeaf);
              j += 2;
            }
    ...
    public IMapEntry find(int shift, int hash, Object key) {
      ...
      if (Util.equiv(key, keyOrNull)) return MapEntry.create(keyOrNull, valOrNode);
      return null;
    }
    ...
    public Object find(int shift, int hash, Object key, Object notFound) {
      ...
      Object keyOrNull = array[2 * idx];
      Object valOrNode = array[2 * idx + 1];
      if (keyOrNull == null) return ((INode) valOrNode).find(shift + 5, hash, key, notFound);
    ...
    private BitmapIndexedNode editAndSet(
          ...
          for (int i = 0; i < 32; i++)
            if (((bitmap >>> i) & 1) != 0) {
              if (array[j] == null) nodes[i] = (INode) array[j + 1];
                ...
                nodes[i] =
                    EMPTY.assoc(edit, shift + 5, hash(array[j]), array[j], array[j + 1], addedLeaf);
              j += 2;
            }
  ...
  static final class HashCollisionNode implements INode {
    ...
    public IMapEntry find(int shift, int hash, Object key) {
      ...
      else return MapEntry.create(array[idx], array[idx + 1]);
    }
    ...
    private HashCollisionNode ensureEditable(
    ...
    private HashCollisionNode editAndSet(
        AtomicReference<Thread> edit, int i, Object a, int j, Object b) {
      HashCollisionNode editable = ensureEditable(edit);
      editable.array[i] = a;
      editable.array[j] = b;
    ...
    public INode assoc(
      ...
      // nest it in a bitmap node
      return new BitmapIndexedNode(
              edit, bitpos(this.hash, shift), new Object[] {null, this, null, null})
  ...
  static final class NodeIter implements Iterator {
    ...
    public boolean hasNext() {
      if (nextEntry != NULL || nextIter != null) return true;
      return advance();
    }
    ...
    public Object next() {
      ...
      if (ret != NULL) {
        ...
        ret = nextIter.next();
        if (!nextIter.hasNext()) nextIter = null;
        return ret;
      } else if (advance()) return next();
      throw new NoSuchElementException();
    }
