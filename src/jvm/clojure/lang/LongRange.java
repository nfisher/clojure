...
public class LongRange extends ASeq implements Counted, IChunkedSeq, IReduce {
  ...
  private LongRange(
      long start, long end, long step, BoundsCheck boundsCheck, LongChunk chunk, ISeq chunkNext) {
    this.start = start;
    this.end = end;
    this.step = step;
    this.boundsCheck = boundsCheck;
  ...
  public static ISeq create(long end) {
    if (end > 0) return new LongRange(0L, end, 1L, positiveStep(end));
    return PersistentList.EMPTY;
  }

  public static ISeq create(long start, long end) {
    if (start >= end) return PersistentList.EMPTY;
  ...
  public ISeq chunkedMore() {
    forceChunk();
    if (_chunkNext == null) return PersistentList.EMPTY;
    return _chunkNext;
  }
  ...
  long steppingCount(long start, long end, long step) {
    ...
    while (count <= CHUNK_SIZE) {
      try {
        ...
        if (boundsCheck.exceededBounds(s)) break;
        else count++;
      } catch (ArithmeticException e) {
        break;
      }
  ...
  public int count() {
    try {
      ...
      if (count > Integer.MAX_VALUE) return Numbers.throwIntOverflow();
  ...
  private static class LongChunk implements IChunk, Serializable {
    ...
    public Object nth(int i, Object notFound) {
      if (i >= 0 && i < count) return start + (i * step);
      return notFound;
    }
    ...
    public LongChunk dropFirst() {
      if (count <= 1) throw new IllegalStateException("dropFirst of empty chunk");
