...
public abstract class APersistentSet extends AFn
  ...
  public static boolean setEquals(IPersistentSet s1, Object obj) {
    ...
    if (m.size() != s1.count()) return false;

    for (Object aM : m) {
      if (!s1.contains(aM)) return false;
    }
  ...
  public Iterator iterator() {
    if (impl instanceof IMapIterable) return ((IMapIterable) impl).keyIterator();
    else
      return new Iterator() {
