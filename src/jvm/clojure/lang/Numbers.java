...
public class Numbers {
  ...
  public static Number rationalize(Number x) {
    if (x instanceof Float || x instanceof Double)
      ...
      BigInteger bv = bx.unscaledValue();
      int scale = bx.scale();
      if (scale < 0) return BigInt.fromBigInteger(bv.multiply(BigInteger.TEN.pow(-scale)));
  ...
  public static Number divide(BigInteger n, BigInteger d) {
    ...
    else if (d.equals(BigInteger.ONE.negate())) return BigInt.fromBigInteger(n.negate());
  ...
  static int hasheqFrom(Number x, Class xc) {
    ...
    if (xc == BigDecimal.class) {
      ...
      if (isZero(x)) return BigDecimal.ZERO.hashCode();
      else {
  ...
  static int hasheq(Number x) {
    ...
    if (xc == Double.class) {
      if (x.equals(-0.0)) return 0; // match 0.0
      return x.hashCode();
    }
  ...
  public static float[] float_array(int size, Object init) {
    ...
    if (init instanceof Number) {
      ...
      for (int i = 0; i < ret.length; i++) ret[i] = f;
    } else {
      ISeq s = RT.seq(init);
  ...
  public static float[] float_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new float[((Number) sizeOrSeq).intValue()];
    else {
  ...
  public static double[] double_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new double[((Number) sizeOrSeq).intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
      double[] ret = new double[size];
  ...
  public static int[] int_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new int[((Number) sizeOrSeq).intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
      int[] ret = new int[size];
  ...
  public static long[] long_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new long[((Number) sizeOrSeq).intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
      long[] ret = new long[size];
  ...
  public static short[] short_array(int size, Object init) {
    ...
    if (init instanceof Short) {
      short s = (Short) init;
      for (int i = 0; i < ret.length; i++) ret[i] = s;
    } else {
      ISeq s = RT.seq(init);
      for (int i = 0; i < size && s != null; i++, s = s.next())
  ...
  public static short[] short_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new short[((Number) sizeOrSeq).intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
      short[] ret = new short[size];
  ...
  public static char[] char_array(int size, Object init) {
    ...
    if (init instanceof Character) {
      ...
      for (int i = 0; i < ret.length; i++) ret[i] = c;
    } else {
      ISeq s = RT.seq(init);
      for (int i = 0; i < size && s != null; i++, s = s.next()) ret[i] = (Character) s.first();
    }
    return ret;
  }
  ...
  public static char[] char_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new char[((Number) sizeOrSeq).intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
      char[] ret = new char[size];
  ...
  public static byte[] byte_array(int size, Object init) {
    ...
    if (init instanceof Byte) {
      byte b = (Byte) init;
      for (int i = 0; i < ret.length; i++) ret[i] = b;
    } else {
      ISeq s = RT.seq(init);
      for (int i = 0; i < size && s != null; i++, s = s.next())
  ...
  public static byte[] byte_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new byte[((Number) sizeOrSeq).intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
      byte[] ret = new byte[size];
  ...
  public static boolean[] boolean_array(int size, Object init) {
    ...
    if (init instanceof Boolean) {
      ...
      ISeq s = RT.seq(init);
      for (int i = 0; i < size && s != null; i++, s = s.next()) ret[i] = (Boolean) s.first();
    }
    return ret;
  }
  ...
  public static boolean[] boolean_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new boolean[((Number) sizeOrSeq).intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
      boolean[] ret = new boolean[size];
  ...
  public static Number minusP(long x) {
    if (x == Long.MIN_VALUE) return BigInt.fromBigInteger(BigInteger.valueOf(x).negate());
    return num(-x);
  }
...
public enum Category {
  ...
  interface Ops {
    ...
    Number unchecked_add(Number x, Number y);

    Number multiply(Number x, Number y);

    Number multiplyP(Number x, Number y);

    Number unchecked_multiply(Number x, Number y);
    ...
    Number unchecked_negate(Number x);
    ...
    Number unchecked_dec(Number x);
  }

  abstract static class OpsP implements Ops {
  ...
  static final class LongOps implements Ops {
    ...
    public final Ops opsWith(BigIntOps x) {
      return BIGINT_OPS;
    }
    ...
    public final Number multiplyP(Number x, Number y) {
      long lx = x.longValue(), ly = y.longValue();
      if (lx == Long.MIN_VALUE && ly < 0) return BIGINT_OPS.multiply(x, y);
      long ret = lx * ly;
  ...
  static final class DoubleOps extends OpsP {
    public Ops combine(Ops y) {
      return y.opsWith(this);
    }
  ...
  static final class RatioOps extends OpsP {
    ...
    public final Number multiply(Number x, Number y) {
      ...
      Number ret =
          Numbers.divide(
              ry.numerator.multiply(rx.numerator), ry.denominator.multiply(rx.denominator));
      return normalizeRet(ret, x, y);
    }
    ...
    public Number divide(Number x, Number y) {
      ...
      Number ret =
          Numbers.divide(
              ry.denominator.multiply(rx.numerator), ry.numerator.multiply(rx.denominator));
      return normalizeRet(ret, x, y);
    }
    ...
    public boolean lt(Number x, Number y) {
      ...
      Ratio ry = toRatio(y);
      return Numbers.lt(
          rx.numerator.multiply(ry.denominator), ry.numerator.multiply(rx.denominator));
    }
  ...
  static final class BigIntOps extends OpsP {
    public Ops combine(Ops y) {
      return y.opsWith(this);
    }
