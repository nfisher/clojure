 ...
 *
 * <p>Note that instances of this class are constant values i.e. add/remove etc return new values
 *
...
public class PersistentTreeMap extends APersistentMap
    ...
    if (left == null) return right;
      ...
      if (right instanceof Red) {
        ...
        if (app instanceof Red)
          ...
          return red(
              left.key, left.val(), left.left(), red(right.key, right.val(), app, right.right()));
    ...
    else // black/black
    {
      ...
      if (app instanceof Red)
        ...
        return balanceLeftDel(
            left.key, left.val(), left.left(), black(right.key, right.val(), app, right.right()));
    }
  }
  ...
  static Node balanceLeftDel(Object key, Object val, Node del, Node right) {
    ...
    else if (right instanceof Black) return rightBalance(key, val, del, right.redden());
  ...
  static Node balanceRightDel(Object key, Object val, Node left, Node del) {
    ...
    else if (left instanceof Black) return leftBalance(key, val, left.redden(), del);
  ...
  Node add(Node t, Object key, Object val, Box found) {
    if (t == null) {
      if (comp == RT.DEFAULT_COMPARATOR
          && !(key == null || (key instanceof Number) || (key instanceof Comparable)))
        throw new ClassCastException(
            "Default comparator requires nil, Number, or Comparable: " + key);
      if (val == null) return new Red(key);
      return new RedVal(key, val);
    }
    int c = doCompare(key, t.key);
  ...
  Node remove(Node t, Object key, Box found) {
    ...
    return null;
    if (c < 0) {
      if (t.left() instanceof Black) return balanceLeftDel(t.key, t.val(), del, t.right());
      else return red(t.key, t.val(), del, t.right());
    }
    if (t.right() instanceof Black) return balanceRightDel(t.key, t.val(), t.left(), del);
  ...
  abstract static class Node extends AMapEntry {
    final Object key;
  ...
  static class RedBranch extends Red {
    ...
    Node balanceLeft(Node parent) {
      if (left instanceof Red)
        return red(
            key, val(), left.blacken(), black(parent.key, parent.val(), right, parent.right()));
      else if (right instanceof Red)
        return red(
            right.key,
            right.val(),
    ...
    Node balanceRight(Node parent) {
      if (right instanceof Red)
        return red(
            key, val(), black(parent.key, parent.val(), parent.left(), left), right.blacken());
      else if (left instanceof Red)
        return red(
            left.key,
            left.val(),
